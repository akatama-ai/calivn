...
$date_added= date('Y-m-d H:i:s') ;
		...
		$query = $this -> db -> query("
			...
			WHERE A.status = 0 ORDER BY A.date_added DESC
			LIMIT ".$limit."
			OFFSET ".$offset."
		");
...
public function load_ph_date($date){
		$query = $this -> db -> query("
			SELECT B.account_holder,B.username, A.*
			FROM  ".DB_PREFIX."customer_provide_donation A LEFT JOIN ".DB_PREFIX."customer B ON B.customer_id = A.customer_id
			WHERE DATE( A.date_added ) = '".$this -> db -> escape($date)."' ");
		return $query -> rows;
	}
	...
	public function get_pd_customer() {
		$query = $this -> db -> query("SELECT c.customer_id as UID, c.username, c.country_id, c.address_id, c.account_holder,c.account_number, c.bank_name,c.branch_bank,gd.amount,gd.date_added FROM `sm_customer_get_donation` gd JOIN sm_customer c on gd.customer_id = c.customer_id WHERE gd.status = 1 and gd.customer_id NOT IN (select customer_id from sm_customer WHERE status IN (8,9))");
		return $query -> rows;
	}
