...
class ControllerSaleCustomer extends Controller {
	...
	*/
	...
	public function confirmPD(){
		$this->load->model('sale/customer');
		
		if (isset($this->request->get['customer_id']) && isset($this->request->get['idtransfer'])  && isset($this->request->get['token'])){

			$linkImage = '';
			$this -> model_sale_customer -> updateStatusPDTransferList($this -> request -> get['idtransfer'],$linkImage);
			// die('999');
			//get PDID
			$Customer_Tranferlist = $this -> model_sale_customer -> getPDByTranferID($this -> request -> get['idtransfer']);
			...
			$this -> model_sale_customer -> updateStusPDActive($PDCustomer, 1);

			$this -> model_sale_customer -> updateCheck_R_WalletPD($PDCustomer);
		
			
			$this -> model_sale_customer -> updateStusGD($GDCustomer);

			// $pd_detail = $this -> model_sale_customer ->getPD_byid($PDCustomer);
			// $customer = $this -> model_sale_customer ->getCustomerfr($pd_detail['customer_id']);
			// $partent = $this->model_sale_customer->getCustomerfr($customer['p_node']);

			// $checkC_Wallet = $this -> model_sale_customer -> checkC_Wallet($partent['customer_id']);

			// if (intval($checkC_Wallet['number']) === 0) {
			// 	if (!$this -> model_sale_customer -> insertC_Wallet($partent['customer_id'])) {
			// 		die();
			// 	}
			// }	
			...
			// $this -> model_sale_customer -> update_C_Walletfr($price, $partent['customer_id']);
			// $this -> model_sale_customer -> saveTranstionHistoryfr($partent['customer_id'], 'C-wallet', '+ ' . number_format($price) . ' VND', "".$partent['username']." Sponsor 10% for F1 ".$customer['username']." finish PD" . $pd_detail['pd_number']." (".number_format($pd_detail['filled'])." VND)");

			//$this -> updatePercent($pd_detail['customer_id'], $pd_detail['filled'], $pd_detail['pd_number']);
			
			$this->session->data['success'] = $this->language->get('Update PD success');
			$url = null;
			if (isset($this->request->get['page'])) {
				$url .= '&page=' . $this->request->get['page'];
			}
	...
	public function updatePercent($customer_id, $amount, $pd_number)
    {
        ...
        $customer = $this -> model_sale_customer -> getCustomerCustom($customer_id);
        $partent = $this -> model_sale_customer -> getCustomerCustom($customer['p_node']);
        
        $priceCurrent = $amount; //(*100 000 000)
        // $price        = ($amount * 0.05);
        
        // $this->model_sale_customer->update_C_Wallet($price, $partent['customer_id']);
      
        // $this->model_sale_customer->saveTranstionHistoryfr($partent['customer_id'], 'C-wallet', '+ ' . number_format($price) . ' VND', "Sponsor 5% for ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
        $levelCustomer = intval($customer['level']);
        $pNode_ID = $partent['customer_id'];
        //F1
        $customerGET = $this->model_sale_customer->getCustomerCustom($pNode_ID);
        if (intval(count($customerGET)) > 0) {
				...
				switch (intval($customerGET['level'])) {
					case 2 :
						if($levelCustomer < 2){
							...
							$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
							$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F1) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
         ...
         //    $percentcommission = $percent / 100;
         //    $this->model_sale_customer->update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
         //    $this->model_sale_customer->saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Sponsor ".$percent."% for ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
					...
					switch (intval($customerGET['level'])) {
						case 2 :
							if($levelCustomer1 < 2){
								...
								$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
								$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F2) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
						...
						switch (intval($customerGET['level'])) {
							case 2 :
								if($levelCustomer2 < 2){
									...
									$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
									$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F3) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
							...
							switch (intval($customerGET['level'])) {
								case 2 :
									if($levelCustomer3 < 2){
										...
										$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
										$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F4) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
								...
								switch (intval($customerGET['level'])) {
									case 2 :
										if($levelCustomer4 < 2){
											...
											$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
											$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F5) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
		            ...
		            $customerGET = $this->model_sale_customer->getCustomerCustom($pNode_ID);
		             if (intval(count($customerGET)) > 0) {
							...
							switch (intval($customerGET['level'])) {
								case 2 :
									if($levelCustomer5 < 2){
										...
										$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
										$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F6) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
			        	...
			        	// =========================
				            ...
				            $customerGET = $this->model_sale_customer->getCustomerCustom($pNode_ID);
				            if (intval(count($customerGET)) > 0) {
				            	$levelCustomer7 = intval($customerGET['level']);
					        	// ==========================
					        	if (intval($customerGET['level']) >= 4) {
					        		if (intval($customerGET['level']) >= $levelCustomer6) {
										$percent = 0.5;
										$percentcommission =0.5/100;
										$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
										$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F7) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
									}else{
										$percent = 0.25;
										$percentcommission =0.25/100;
										$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
				            ...
				            $customerGET = $this->model_sale_customer->getCustomerCustom($pNode_ID);
				            if (intval(count($customerGET)) > 0) {
				            	$levelCustomer = intval($customerGET['level']);
					        	// ==========================
					        	if (intval($customerGET['level']) >= 4) {
									if (intval($customerGET['level']) >= $levelCustomer) {
										$percent = 0.5;
										$percentcommission =0.5/100;
										$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
										$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F8) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
									}else{
										$percent = 0.25;
										$percentcommission =0.25/100;
										$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
					        	...
					        	// ==========================
					        		if (intval($customerGET['level']) >= 4) {
					        			if (intval($customerGET['level']) >= $levelCustomer) {
											...
											$percentcommission =0.5/100;
											$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
											$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F9) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
										}else{
											$percent = 0.25;
											$percentcommission =0.25/100;
											$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
					        	...
					        	// ==========================
					        			if (intval($customerGET['level']) >= 4) {
											if (intval($customerGET['level']) >= $levelCustomer) {
												...
												$percentcommission =0.5/100;
												$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
												$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F10) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
											}else{
												$percent = 0.25;
												$percentcommission =0.25/100;
												$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
					        	...
					        	// ==========================
					        				if (intval($customerGET['level']) >= 4) {
					        					if (intval($customerGET['level']) >= $levelCustomer) {
													...
													$percentcommission =0.5/100;
													$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
													$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F11) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
												}else{
													$percent = 0.25;
													$percentcommission =0.25/100;
													$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
					        	...
					        	// ==========================
					        					if (intval($customerGET['level']) >= 4) {
					        						if (intval($customerGET['level']) >= $levelCustomer) {
														...
														$percentcommission =0.5/100;
														$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
														$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F12) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
													}else{
														$percent = 0.25;
														$percentcommission =0.25/100;
														$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
					        	...
					        	// ==========================
					        						if (intval($customerGET['level']) >= 4) {
					        							if (intval($customerGET['level']) >= $levelCustomer) {
															...
															$percentcommission =0.5/100;
															$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
															$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F13) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
														}else{
															$percent = 0.25;
															$percentcommission =0.25/100;
															$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
					        	...
					        	// ==========================
											        	if (intval($customerGET['level']) >= 4) {
											        		if (intval($customerGET['level']) >= $levelCustomer) {
																...
																$percentcommission =0.5/100;
																$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
																$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F14) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
															}else{
																$percent = 0.25;
																$percentcommission =0.25/100;
																$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
											            ...
											            if (intval(count($customerGET)) > 0) {
												        	...
												        	if (intval($customerGET['level']) >= 4) {
												        		if (intval($customerGET['level']) >= $levelCustomer) {
																	...
																	$percentcommission =0.5/100;
																	$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
																	$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F15) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
																}else{
																	$percent = 0.25;
																	$percentcommission =0.25/100;
																	$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
													            ...
													            if (intval(count($customerGET)) > 0) {
														        	...
														        	if (intval($customerGET['level']) >= 4) {
																		if (intval($customerGET['level']) >= $levelCustomer) {
																			...
																			$percentcommission =0.5/100;
																			$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
																			$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F16) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
																		}else{
																			$percent = 0.25;
																			$percentcommission =0.25/100;
																			$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
														            ...
														            if (intval(count($customerGET)) > 0) {
															        	...
															        	if (intval($customerGET['level']) >= 4) {
																			if (intval($customerGET['level']) >= $levelCustomer) {
																				...
																				$percentcommission =0.5/100;
																				$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
																				$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F17) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
																			}else{
																				$percent = 0.25;
																				$percentcommission =0.25/100;
																				$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
															            ...
															            if (intval(count($customerGET)) > 0) {
																        	...
																        	if (intval($customerGET['level']) >= 4) {
																        		if (intval($customerGET['level']) >= $levelCustomer) {
																						...
																						$percentcommission =0.5/100;
																						$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
																						$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F18) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
																					}else{
																						$percent = 0.25;
																						$percentcommission =0.25/100;
																						$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
																            ...
																            if (intval(count($customerGET)) > 0) {
																	        	...
																	        	if (intval($customerGET['level']) >= 4) {
																	        		if (intval($customerGET['level']) >= $levelCustomer) {
																						...
																						$percentcommission =0.5/100;
																						$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
																						$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F19) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
																					}else{
																						$percent = 0.25;
																						$percentcommission =0.25/100;
																						$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
				            ...
				            //F20
																	            $customerGET = $this->model_sale_customer->getCustomerCustom($pNode_ID);
																	            if (intval(count($customerGET)) > 0) {
																	            	$levelCustomer20 = intval($customerGET['level']);
																		        	// ==========================
																		        	if (intval($customerGET['level']) >= 4) {
																						if (intval($customerGET['level']) >= $levelCustomer) {
																							$percent = 0.5;
																							$percentcommission =0.5/100;
																							$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
																							$this -> model_sale_customer -> saveTranstionHistoryfr($customerGET['customer_id'], 'C-wallet', '+ ' . number_format($priceCurrent * $percentcommission) . ' VND', "".$customerGET['username']." Earn ".$percent." % commission  for (F20) - ".$customer['username']." finish PD" . $pd_number." (".number_format($amount)." VND)");
																						}else{
																							$percent = 0.25;
																							$percentcommission =0.25/100;
																							$this -> model_sale_customer -> update_C_Walletfr($priceCurrent * $percentcommission, $customerGET['customer_id']);
	...
	//	$data['nameParrent']= $this->model_sale_customer->getNameparrent($data['pnode']);
		...
		$data['cwallet'] = number_format($data['cwallet']);

		$data['rwallet'] = $this->model_sale_customer->getR_Wallet($customer_info['customer_id']);
		$data['rwallet'] = number_format($data['rwallet']) ;		
